...
namespace eProdaja.WinUI
{
    public class APIService
    {
        ...
        public string _endpoint = "https://localhost:7184/";
        ...
        {
            var result = await $"{_endpoint}{_resource}".WithBasicAuth(Username, Password).PostJsonAsync(requst).ReceiveJson<T>();
            return result;
        }

        public async Task<T> Put<T>(object id, object requst)
        {
            var result = await $"{_endpoint}{_resource}/{id}".WithBasicAuth(Username, Password).PutJsonAsync(requst).ReceiveJson<T>();
            return result;
        }
    }
}
